...
def fetch_vn_stocks(**context):
    ...
    for _, row in df.iterrows():
        records.append({
            ...
            'name_en': str(row['organ_name']) if pd.notna(row['organ_name']) else '',
            'name_local': str(row['organ_name']) if pd.notna(row['organ_name']) else '',
            'asset_class': 'STOCK',       # STI discriminator
            'market': 'VN',               # Market code
            'currency': 'VND',
            'exchange': str(row['exchange']) if pd.notna(row['exchange']) else 'HOSE',
            'sector': str(row['icb_name2']) if pd.notna(row['icb_name2']) else 'Unknown',
            'industry': str(row['icb_name3']) if pd.notna(row['icb_name3']) else 'Unknown',
            'logo_url': '',
            'description': '',
...
def fetch_us_stocks(**context):
    ...
    for idx, row in sp500_df.iterrows():
        ...
        try:
            ...
            enriched_data.append({
                ...
                'industry': info.get('industry', row.get('GICS Sub-Industry', 'Unknown')),
    ...
    # Step 3: Sort by market cap, take top 50
    df = pd.DataFrame(enriched_data)
    df_top50 = df.nlargest(50, 'market_cap')
    ...
    for _, row in df_top50.iterrows():
        records.append({
            ...
            'currency': 'USD',
            'exchange': str(row['exchange']) if pd.notna(row['exchange']) else 'NASDAQ',
            'sector': str(row['sector']) if pd.notna(row['sector']) else 'Unknown',
            'industry': str(row['industry']) if pd.notna(row['industry']) else 'Unknown',
            'logo_url': '',
            'description': '',
...
def fetch_crypto(**context):
    """
    Fetch top 50 crypto from CoinGecko
    ...
    logger.info("Fetching top 50 crypto from CoinGecko...")
