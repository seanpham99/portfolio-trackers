...
def sync_assets(**context):
    ...
    if not url or not key:
        raise ValueError("SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY environment variable not set")
    ...
    for row in rows:
        ...
        asset_data = {
            ...
            'metadata': row[col_idx['external_api_metadata']] or {}, # supabase-py handles dict -> jsonb
    ...
    logger.info(f"Starting UPSERT of {len(assets_to_sync)} records in batches of {batch_size}...")
    ...
    for i in range(0, len(assets_to_sync), batch_size):
        ...
        try:
            ...
            response = supabase.table('assets').upsert(
                batch,
                on_conflict='symbol,market,asset_class'
            ).execute()
            
            total_synced += len(batch)
            logger.info(f"Synced batch {i//batch_size + 1}: {total_synced}/{len(assets_to_sync)}")
            
        except Exception as e:
            logger.error(f"❌ Failed to sync batch starting at index {i}: {e}")
            raise
            
    logger.info(f"✅ Successfully synced {total_synced} assets to Supabase using Supabase Client")
    return total_synced
